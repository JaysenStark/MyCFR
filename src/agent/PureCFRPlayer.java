...
public class PureCFRPlayer {
	...
	public static void main(String args[]) {
		...
		if (args.length != 4) {
			System.out
					.println("Usage: pure_cfr_player <player_file> <server> <port>\n");
		}
		...
		try {
			...
			try {
				...
				while ((serverStr = br.readLine()) != null) {
					...
					/*
					 * Start building the response to the server by adding a
					 * colon (guaranteed to fit because we read a new-line in
					 * fgets)
					 */
